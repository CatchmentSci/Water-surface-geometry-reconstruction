...
wavelengthFilter = struct( ...
    'minWavelength_m', 0.5, ...
    'maxWavelength_m', 7, ...
    'minAutocorrPeakR', 0.10, ...
    'madScaleFactor', 1.4826, ...
    'scaledMadMultiplier', 3.5, ...
    'minProfilesForMad', 8);
...
function lambda = accepted_autocorr_wavelengths(lambda, peakR, filter)
lambda = double(lambda(:));
peakR = double(peakR(:));
lambda(~isfinite(lambda) | lambda < filter.minWavelength_m | ...
    lambda > filter.maxWavelength_m) = NaN;
lambda(~isfinite(peakR) | peakR < filter.minAutocorrPeakR) = NaN;
idx = find(isfinite(lambda));
if numel(idx) >= filter.minProfilesForMad
    values = lambda(idx);
    centre = median(values, 'omitnan');
    scaledMad = filter.madScaleFactor .* ...
        median(abs(values-centre), 'omitnan');
    if isfinite(scaledMad) && scaledMad > 0
        lambda(idx(abs(values-centre) > ...
            filter.scaledMadMultiplier.*scaledMad)) = NaN;
    end
end
end
