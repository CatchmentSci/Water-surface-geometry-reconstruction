...
%   accepted-autocorrelation wavelengths, accepted-map amplitudes, and the
%   initial-planar-velocity inverse-depth summary for stable cases R1-R11.
...
validate_wavelength_filter(loaded.wavelengthFilter);
...
assert_fields(velocity, ["caseNumber", "caseLabel", "deep_xMedian", ...
    "deep_yMedian", ...
...
for ii = 1:height(velocity)
    T = transects{ii};
    assert_fields(T, ["caseLabel", "commonValid", ...
        "acceptedAutocorrWavelength_m", ...
        "initialVelocityTrackedMedian_mps", "velocityDeepWater_mps"], ...
        sprintf("%s transect table", labels(ii)));
    ...
    valid = logical(T.commonValid) & ...
        ...
        isfinite(double(T.initialVelocityTrackedMedian_mps)) & ...
        isfinite(double(T.velocityDeepWater_mps));
...
if any(abs(UfromLambda_mps(included) - ...
        double(velocity.deep_yMedian(included))) >= 5e-4)
...
surfaceVelocityKLT_mps = double(velocity.deep_xMedian(included));
...
tableData = table(caseNumber, caseLabel, surfaceVelocityKLT_mps, ...
    lambdaEst_m, amplitudeEst_m, ...
...
write_line(fid, "\begin{tabular}{c c c c c c c c c c c}");
write_line(fid, "\toprule");
write_line(fid, "& \multicolumn{3}{c}{Estimated}");
write_line(fid, "& \multicolumn{4}{c}{Derived velocity}");
write_line(fid, "& \multicolumn{3}{c}{Derived depth} \\");
write_line(fid, "\cmidrule(lr){2-4}");
write_line(fid, "\cmidrule(lr){5-8}");
write_line(fid, "\cmidrule(lr){9-11}");
write_line(fid, "Case");
write_line(fid, "& $U_s$");
...
for ii = 1:height(T)
    fields = [sprintf("R$_{%d}$", T.caseNumber(ii)), ...
        format_value(T.surfaceVelocityKLT_mps(ii)), ...
...
caption = [ ...
    ...
    "hydraulic descriptors. $U_s$ is the case median of the transect-scale " + ...
    "streamwise surface velocities obtained using KLT-IV, calculated over " + ...
    "the same valid transects used for the wavelength-derived velocity " + ...
    "estimates. $\lambda_{\mathrm{est}}$ is the median " + ...
    ...
    "\ref{eq:wavec}). Depth inversion uses the tracked streamwise velocity " + ...
    "projected onto the initial planar water surface. Derived depths use " + ...
    "the deep solution branch, or the " + ...
...
function validate_wavelength_filter(filter)
...
if ~isstruct(filter) || any(~isfield(filter, cellstr(required))) || ...
    ...
    error("TableC1:UnexpectedWavelengthFilter", ...
