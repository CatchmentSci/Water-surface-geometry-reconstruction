...
#   sbatch --job-name=klt_s1 --export=ALL,KLT_S_ID=1 run_one_klt_case.sh
...
if [[ "$S_ID" -ge 2 && "$S_ID" -le 8 ]]; then
    ...
    INPUT_MAT_FILE="${SYN_DIR}/solver_inputs_s1pt79.mat"
else
...
try
    ...
    %
    % If the KLT code itself starts or requires a pool, this leaves it free
    % to use the site's default MATLAB parallel settings. If you later find
    % that no pool is being started, add a minimal "parpool" here only after
    % confirming the site-recommended profile.
    pool = gcp('nocreate');
    if isempty(pool)
