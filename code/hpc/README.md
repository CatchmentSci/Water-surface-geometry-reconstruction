...
| File | Purpose |
|---|---|
| `syn/submit_klt_syn.sh` | Submits the configured set of synthetic cases as individual Slurm jobs. |
| `syn/resubmit_missing_klt_jobs.sh` | Checks whether named cases are active and resubmits missing jobs. |
| `syn/run_one_klt_case.sh` | Runs one case selected by the exported `KLT_S_ID` value. |
| `syn/run_one_klt_case_ramlog.sh` | Runs one case while recording MATLAB and descendant-process memory use. |
...
The case CSV and solver-input MAT files are part of the associated Zenodo dataset rather than this Git repository. The four scripts under `syn` are byte-for-byte copies of the operational files under `/mnt/nfs/home/nmp65/slurm_commands/synthetic` on Comet; the three scripts and lookup under `real` match `/mnt/nfs/home/nmp65/slurm_commands/real`.

`syn/run_one_klt_case_ramlog.sh`, selected by the operational `syn/submit_klt_syn.sh`, uses Excel-visible row numbers from `synthetic_cases.csv` (row 1 is the header) and maps them as follows:
...
The active `S_IDS` value in `syn/submit_klt_syn.sh` records the selected operational run and does not define the complete dataset. `syn/run_one_klt_case.sh` and `syn/resubmit_missing_klt_jobs.sh` are also retained exactly from Comet; the standard worker contains an earlier row-range configuration and should not be substituted for the RAM-logging worker when reproducing the final 39-case mapping.
